...
<?php //Creation date:  2024-07-12 03:07:09    ?>
...
<?php
if (_options_option("config_hr_fines_categories_show_col_from_table")) {
    $colsToShow = json_decode(_options_option("config_hr_fines_categories_show_col_from_table"), true);
...
<table class="table table-striped">
    <thead>
        <tr class="info">            
            <?php hr_fines_categories_index_generate_column_headers($cols_to_show_keys); ?>                                                                              
        </tr>
    </thead>
    <tfoot>
        <tr class="info">
    ...
    <tbody>
        ...
        foreach ($hr_fines_categories as $hr_fines_categories_item) {
            ...
            foreach ($cols_to_show_keys as $key => $col) {
                ...
                switch ($col) {
                    case 'id':
                        echo '<td>' . ($hr_fines_categories_item[$col]) . '</td>';
                        break;
                    case 'category_code':
                        echo '<td>' . ($hr_fines_categories_item[$col]) . '</td>';
                        break;
...
//                echo '</td>';
    ...
    <?php // include view("hr_fines_categories", "table_index_form_add");  ?>
