...
<form class="form-horizontal" action="index.php" method="post" >
    ...
    <?php
//vardump(hr_employee_payroll_items_list_by_employee($id)); 
    ?>
        ...
        <div class="col-sm-8">
            <select class="form-control" name="code" id="code">
                ...
                foreach (hr_payroll_items_without_formula_not_used_by($id) as $key => $hrpril) {
                    ...
                    //$hrpril_disabled = (in_array($hrpril['code'], hr_employee_payroll_items_list_by_employee_id($id))) ? " disabled " : "";

                    echo '<option value="' . $hrpril['code'] . '"   ' . $hrpril_disabled . '>' . $hrpril['code'] . ' : ' . $hrpril['description'] . '</option>';
                }
                ?>
            </select>
        </div>	
