...
<table class="table table-striped table-condensed table-bordered" >
    ...
    <tbody>
        ...
        foreach (banks_lines_search_by_status(40) as $key => $bll) {
            ...
            if (abs($bll['total']) == $hr_payroll->getTo_pay()) {

                $bl = new Banks_lines();
                $bl->setBanks_lines($bll["id"]);

                $bl_class_date_operation = ($bl->getDate_operation() == $hr_payroll->getDate_start()) ? ' class="warning" ' : '';

                $bl_class_total = ( abs($bll['total']) == abs($hr_payroll->getTo_pay()) ) ? ' class="text-right warning" ' : ' class="text-right" ';

                //$bl_class_ce = ($bl->getCe() == $hr_payroll->getCe()) ? ' class="warning" ' : '';
